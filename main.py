...
def main():
    ...
    print(f"Procesando fecha: {query_date.strftime('%Y-%m-%d')}, turno: {query_shift}")

    # Get a list of nurses for a specific shift
    date, shift, nurses = get_nurse_shift(config['excel_info'])

    # Get historic data for the given date, shift, nurses, and patients
    historic, occupied_rooms, nurses_per_control, historic_summary_a_b = get_historic(date, shift, nurses, config['paths_historic'])
    ...
    # Assign nurses to the distributed rooms based on historic data
    best_mapping = assign_nurses(distributed_rooms, nurses, historic, historic_summary_a_b, occupied_rooms, query_date)
